...
class PokerHand
  ...
  def royal_flush? # All cards of the same suit and in sequence, starting from 10 -> Ace
  ...
  def flush? # Five cards all of the same suit
    suit = @cards.first.suit
    @cards.all? {|card| card.suit == suit}
  end

  def straight? # Five cards in sequence
    return false if @rank_count.any? {|_, count| count > 1}
    @cards.min.score == @cards.max.score - 4
  end
